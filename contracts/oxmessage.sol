...
contract oxmessage {
    ...
    struct Message{
        uint timestamp; // set in contract
        address sender;
        address receiver;
    ...
    event MessagePosted(address indexed from, address indexed to, uint256 receiverIndex);
    ...
    Message memory message = Message(block.timestamp, msg.sender, to, content);
    message.timestamp = block.timestamp;
    // add to send and receive mappings, update counts 
    uint idx = _sentMessagesCount[msg.sender];
    ...
    function receivedCount(address addr)  
    external view 
    returns( uint256 )
    {
    ...
    function receivedMessage(address addr, uint idx)  
    external view 
    ...
    function sentCount(address addr)  
    external view 
    returns( uint256 )
    {
    ...
    function sentMessage(address addr,uint idx)  
    external view 
    returns( Message memory )
    {
